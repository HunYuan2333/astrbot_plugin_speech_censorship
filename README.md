...
- 🧠 **检索增强**：支持“黑话/自定义屏蔽词”检索增强，命中后仍由 LLM 复核再处罚
- ⚡ **灵活触发**：支持四种检测模式（仅定时、仅消息数量、混合模式、严格混合模式）
...
```
/slang_add 阴阳怪气 --aliases=yygq,阴阳 --severity=high
```

录入并补充语义信息：

```
/slang_add 六个鸡蛋 --category=political_metaphor --hint="常用于政治影射语境" --examples="我就爱吃六个鸡蛋,他总在提六个鸡蛋" --severity=high
```

录入为全局词条：

```
/slang_add 黑称 --global --severity=medium
```

查看候选新黑话（管理员）：

```
/slang_candidates --limit=10
```

一键转正候选词条（管理员）：

```
/slang_promote 六个鸡蛋 --severity=high
```
...
- **触发模式**：
  ...
  - `strict_hybrid`：严格混合（时间和数量都达标才触发，更省 token）

- **检测时间间隔**：定时触发的时间间隔（秒），默认 60 秒

- **触发检测的消息数量**：累积多少条消息后触发检测，默认 10 条

- **严格混合最近消息窗口**：仅 `strict_hybrid` 生效，只保留最近 N 条消息用于缓冲与送审；设为 0 表示不限制
...
- **黑话功能总开关**：关闭后不拼接任何黑话相关提示词（检索增强与候选发现均停用）
- **启用黑话检索增强**：开启后会在 LLM 审核前注入词库命中证据
- **黑话检索区分大小写**：默认关闭（不区分大小写）
- **单次审核最多注入检索证据条数**：默认 12，防止提示词过长

### 候选新黑话发现配置

- **启用候选新黑话自动发现**：在常规审核时，让 LLM 同步返回疑似新黑话
- **候选新黑话最小置信度**：低于阈值的候选不会保存
- **单次审核最多保存候选数**：防止一次性写入过多低质量候选
- **候选新黑话发现提示词**：自定义候选发现任务的提示词，可根据实际场景调优（如专注特定类型黑话、调整置信度判断标准等）

> **提示词自定义技巧**：
>
> - 针对特定平台：如果是游戏群，可以强调识别"游戏术语衍生黑话"
> - 调整置信度标准：根据群氛围调整政治隐喻的置信度阈值建议
> - 简化或详化：可以精简提示词以节省 token，或增加更多示例以提高识别准确度
> - 关注区域特色：可以添加特定区域的黑话特征（如方言、地域梗）

> 词库数据文件位置：`data/plugin_data/<plugin_name>/slang_entries.json`
>
> 支持“全局词条 + 群覆盖词条”，并包含生效时间、失效时间、版本、来源等治理字段。
>
> 候选新黑话文件位置：`data/plugin_data/<plugin_name>/slang_candidates.json`
